...
const DiagnosticsScreen: React.FC = () => {
  ...
  const { data, isFetching, isError, refetch } = useGetCharactersQuery({
    page: 1,
    name: '',
    status: '' as any,
  });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text>
          {isFetching
            ? 'Cargando...'
            : `Personajes (total info.count): ${data?.info?.count ?? '—'}`}
        </Text>
        {!!isError && <Text style={styles.error}>Error al consultar API</Text>}
      ...
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          <View style={styles.btn}>
            <Button title="Crear nota" onPress={createNote} />
          </View>
          <View style={styles.btn}>
            <Button title="Borrar todo" color="#C03" onPress={clearNotes} />
          </View>
        </View>
      </View>
